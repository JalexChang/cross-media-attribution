...
class TestSVD(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = SVD(max_iters=1000,alpha=0.001,beta=0.003, item_biased =True, user_biased = True)
        self.model.load_dataset(self.dataset)
    
    def test_init(self):
        self.assertIsInstance(self.model,SVD)

    def test_init_with_agrv(self):
        model = SVD(max_iters=500,alpha=0.003,beta=0.1,delta=0.0001,verbose=True)
