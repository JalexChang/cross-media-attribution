...
class Profile:
    ...
    @classmethod
    def factor_dist_on_users(self, dataset):
        matrix_shape = ( dataset.matrix_shape()[0],dataset.matrix_shape()[1])
        user_num_factors = numpy.zeros(matrix_shape[0], dtype=int)
        dist = numpy.zeros(matrix_shape[1], dtype=int)

        user_factor_matrix = dataset.touchs.to_matrix()
        for u_id in range(matrix_shape[1]):
    ...
    @classmethod
    def factor_dist_on_items(self, dataset):
        matrix_shape = ( dataset.matrix_shape()[2],dataset.matrix_shape()[1])
        item_factor_table  = numpy.zeros(matrix_shape, dtype=bool)
        item_num_factors = numpy.zeros(matrix_shape[0], dtype=int)
        dist = numpy.zeros(matrix_shape[1], dtype=int)
        user_factor_matrix = dataset.touchs.to_matrix()

        for u_id, i_id, rating in dataset.ratings.to_list():
            for f_id in range(matrix_shape[1]):
